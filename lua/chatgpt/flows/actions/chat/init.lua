...
local function get_quickfix_entries(lines, filename)
  local entries = {}
  for _, line in ipairs(lines) do
    local lnum, text = line:match("(%d+):(.*)")

    local entry = { filename = filename, lnum = tonumber(lnum), text = text }
...
function ChatAction:run()
  ...
  vim.schedule(function()
    ...
    Api.chat_completions(params, function(answer, usage)
      self:on_result(answer, usage, filename)
    end)
  end)
end

function ChatAction:on_result(answer, usage, filename)
  vim.schedule(function()
    ...
    elseif self.strategy == STRATEGY_QUICK_FIX then
      ...
      local entries = get_quickfix_entries(lines, filename)
